  ...
  const toggleUrgent = async (e: React.MouseEvent) => {
    ...
    toast.promise(
      async () => {
        ...
        // Refresh the server-side props
        router.refresh();
      },
      {
        loading: "Updating urgency status...",
        success: () =>
          `Report #${report.id} ${
            newIsUrgent ? "marked" : "unmarked"
          } as urgent`,
        error: "Failed to update urgency status",
      }
    );
  };
  ...
  const toggleBookmark = async (e: React.MouseEvent) => {
    ...
    toast.promise(
      ...
      },
      {
        ...
        success: () =>
          `Report #${report.id} ${
            newIsBookmarked ? "added to" : "removed from"
          } bookmarks`,
      ...
      <Link href={`/dashboard/${report.id}`} className="block p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">{firstLine}</p>
        ...
        <div className="relative">
          <p className="text-sm text-gray-500 line-clamp-6">{previewContent}</p>
